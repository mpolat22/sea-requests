...
const searchQuery = ref(props.rfqsTable.filters?.search ?? '');
...
const copy = {
    ...
    searchPlaceholder: 'Search ref / buyer / ship / type / access',
    search: 'Search',
    table: {
        order: '#',
        statusMini: 'Status',
    ...
    compareCompleted: 'This RFQ is completed. Continue from RFQ detail or Orders.',
...
const totalRecords = computed(() => Number(meta.value.total ?? rfqs.value.length));
...
const visibilityLabel = (visibilityScope) => (
    visibilityScope === 'private_supplier'
        ? copy.private
        : copy.marketplace
);
...
watch(() => props.rfqsTable.filters, (value) => {
    searchQuery.value = value?.search ?? '';
    rowsPerPage.value = Number(value?.per_page ?? props.rfqsTable.meta?.per_page ?? 10);
}, { deep: true });
</script>

<template>
    <AdminDashboardShell :dashboard="dashboard" :title="copy.title" active-tab="rfqs">
                        ...
                        <tr v-for="(rfq, index) in rfqs" :key="rfq.id">
                            ...
                            <td>
                                <div class="reference-cell">
                                    <Link :href="rfq.show_url" class="reference-link">
                                        {{ rfq.reference_no }}
                                    </Link>
                                    <span class="reference-meta">
                                        {{ requestTypeLabel(rfq.request_type) }} • {{ visibilityLabel(rfq.visibility_scope) }}
                                    </span>
                                </div>
                            </td>
                            <td>{{ rfq.company_name || copy.noData }}</td>
                            <td>{{ rfq.ship_name || copy.noData }}</td>
                            ...
                            <td>
                                <span class="pill pill-urgency" :class="urgencyTone(rfq.priority)">
                                    {{ capitalizeLabel(copy.priority[rfq.priority] || rfq.priority || copy.noData) }}
                                </span>
                            </td>
                            <td>
                                <Link
                            ...
                            <td>
                                <div class="actions-cell">
                                    <Link :href="rfq.show_url" class="action-button action-view" :title="copy.view">
                                    ...
                                    <Link
                                        v-if="canCompareOffers(rfq)"
                                        :href="rfq.compare_url"
                                        class="action-button action-compare"
                                        :title="compareTitle(rfq)"
                                    >
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <path d="M4 7h16" />
                ...
                <article v-for="(rfq, index) in rfqs" :key="`mobile-${rfq.id}`" class="mobile-record-card">
                    ...
                    <div class="mobile-card-grid">
                        <div class="mobile-card-field">
                            ...
                            <span class="mobile-field-value">{{ rfq.company_name || copy.noData }}</span>
                        </div>
                        <div class="mobile-card-field">
                            <span class="mobile-field-label">{{ copy.table.ship }}</span>
                            <span class="mobile-field-value">{{ rfq.ship_name || copy.noData }}</span>
                        </div>
                        <div class="mobile-card-field">
        ...
        <div v-if="deleteModalRfq" class="delete-modal-backdrop" @click.self="closeDeleteModal">
            <div class="delete-modal">
                ...
                <h3 class="directory-section-title">{{ copy.deleteTitle }}</h3>
                <p class="delete-modal-copy">{{ copy.deleteBody }}</p>
                <div class="delete-modal-summary">
                    <span>{{ deleteModalRfq.reference_no }}</span>
                    <span>{{ deleteModalRfq.company_name || copy.noData }}</span>
                    <span>{{ deleteModalRfq.ship_name || copy.noData }}</span>
                </div>
...
.reference-cell{display:grid;gap:4px}
.reference-link{color:#0f172a;font-weight:600;text-decoration:none}
.reference-meta{color:#64748b;font-size:.78rem;line-height:1.45;white-space:normal}
...
@media (max-width: 900px){
    ...
    .toolbar-search{flex-direction:column;align-items:stretch;margin-left:0}
